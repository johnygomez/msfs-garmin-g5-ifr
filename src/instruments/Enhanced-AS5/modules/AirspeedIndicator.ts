...
export class AirspeedIndicator extends HTMLElement {
    bottomBackground: HTMLElement
    ...
    root: Element
    airspeedReferenceGroup: Element
    selectedSpeedFixedBug: Element
    selectedSpeedText: Element
    selectedSpeedTextMach: Element
    centerSvg: Element
    centerGroup: Element
    gradTexts: any[]
    redElement: Element
    yellowElement: Element
    greenElement: Element
    flapsElement: Element
    startElement: Element
    endElement: Element
    vyseElement: Element
    vmcElement: Element
    selectedSpeedBug: Element
    cursor: Element
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        const centerY = (this.height - 100) / 2
        switch (name) {
            case 'airspeed':
                this.value = Math.max(parseFloat(newValue), 20)
                const center = Math.max(Math.round(this.value / 10) * 10, 60)
                if (
                    (!this.nocolor && this.minValue > 0 && this.value < this.minValue) ||
                    (this.maxValue > 0 && this.value > this.maxValue)
                ) {
                ...
                diffAndSetAttribute(
                    this.centerGroup,
                    'transform',
                    'translate(0, ' + (this.value - center) * 10 + ')'
                )
                if (!this.nocolor) {
                    if (this.minValue > 0) {
                        const val =
                            this.height +
                            200 +
                            (center - this.minValue + (this.height - 100) / 20) * 10 +
                            (this.value - center) * 10
                        diffAndSetAttribute(
                            this.startElement,
                            'transform',
                            'translate(0,' + val + ')'
                    ...
                    if (this.maxValue > 0) {
                        const val =
                            100 +
                            Utils.Clamp(
                                (center - this.maxValue + (this.height - 100) / 20) * 10,
                                -100,
                                this.height + 100
                            ) +
                            (this.value - center) * 10
                ...
                for (let i = 0; i < this.referenceBugs.length; i++) {
                    diffAndSetAttribute(
                        this.referenceBugs[i].group,
                        'transform',
                        'translate(0,' + (this.value - this.referenceBugs[i].value) * 10 + ')'
                    )
                }
                diffAndSetAttribute(
                    this.selectedSpeedBug,
                    'transform',
                    'translate(0,' + (this.value - this.selectedSpeedBugValue) * 10 + ')'
                )
                if (this.currentCenterGrad != center) {
                    ...
                    if (!this.nocolor) {
                        const greenEnd = Math.min(
                            Math.max(-100, centerY + -10 * (this.greenEnd - center)),
                            this.height
                        )
                        const greenBegin = Math.min(
                            Math.max(-100, centerY + -10 * (this.greenBegin - center)),
                            this.height
                        )
                        diffAndSetAttribute(this.greenElement, 'y', greenEnd + '')
                        diffAndSetAttribute(this.greenElement, 'height', greenBegin - greenEnd + '')
                        const yellowEnd = Math.min(
                            Math.max(-100, centerY + -10 * (this.yellowEnd - center)),
                            this.height
                        )
                        const yellowBegin = Math.min(
                            Math.max(-100, centerY + -10 * (this.yellowBegin - center)),
                            this.height
                        )
                        ...
                        const redEnd = Math.min(
                            Math.max(-100, centerY + -10 * (this.redEnd - center)),
                            this.height
                        )
                        const redBegin = Math.min(
                            Math.max(-100, centerY + -10 * (this.redBegin - center)),
                            this.height
                        )
                        diffAndSetAttribute(this.redElement, 'y', redEnd + '')
                        diffAndSetAttribute(this.redElement, 'height', redBegin - redEnd + '')
                        const flapsEnd = Math.min(
                            Math.max(-100, centerY + -10 * (this.flapsEnd - center)),
                            this.height
                        )
                        const flapsBegin = Math.min(
                            Math.max(-100, centerY + -10 * (this.flapsBegin - center)),
                            this.height
                        )
                        ...
                        if (this.vyseValue > 10) {
                            const vyseEnd = Math.min(
                                Math.max(-100, centerY + -10 * (this.vyseValue - center)) - 4,
                                this.height
                            )
                            diffAndSetAttribute(this.vyseElement, 'y', vyseEnd.toFixed(0))
                        }
                        if (this.vmcValue > 10) {
                            const vmcEnd = Math.min(
                                Math.max(-100, centerY + -10 * (this.vmcValue - center)) - 4,
                                this.height
                            )
                ...
                diffAndSetAttribute(
                    this.endDigitsGroup,
                    'transform',
                    'translate(0, ' + (endValue - endCenter) * this.endDigitSpace + ')'
                )
                ...
                if (this.value > 20) {
                    ...
                    if (Math.abs(this.value) >= 99) {
                        ...
                        diffAndSetText(
                            this.digit1Bot,
                            Math.abs(this.value) < 100 ? '' : fastToFixed(Math.floor(d1Value), 0)
                        )
                        diffAndSetText(
                            this.digit1Top,
                ...
                break
            case 'airspeed-trend':
                this.trendValue = Math.min(
                    Math.max(centerY + parseFloat(newValue) * 6 * -10, 0),
                    this.height - 100
                )
                ...
                diffAndSetAttribute(
                    this.trendElement,
                    'height',
                    Math.abs(this.trendValue - centerY) + ''
                )
                break
            case 'min-speed':
                ...
                for (let i = 0; i < elements.length; i++) {
                    if (i >= this.referenceBugs.length) {
                        ...
                        diffAndSetAttribute(
                            newRef.bug,
                            'points',
                            '200,300 210,315 250,315 250,285 210,285'
                        )
    ...
    connectedCallback() {
        ...
        this.appendChild(this.root)
        {
            ...
            diffAndSetAttribute(
                this.selectedSpeedFixedBug,
                'points',
                '190,-40 180,-40 180,-30 185,-25 180,-20 180,-10 190,-10 '
            )
