...
export class AttitudeIndicator extends HTMLElement {
    topTriangle: any
    ...
    turnRateIndicatorShown: boolean
    turnRateIndicatorMarkerX: number
    flightDirectorActive: boolean
    flightDirectorPitch: number
    flightDirectorBank: number
    lowBankModeHeight: number
    lowMaxBankAngle: number
    aspectRatio: number
    isBackup: boolean
    horizonTopColor: string
    horizonBottomColor: string
    horizonTopColorLight: string
    horizonBottomColorLight: string
    horizonTopGradient: string
    horizonBottomGradient: string
    GF_font: string
    ...
    horizonTop: Element
    bottomPart: Element
    horizonBottom: Element
    root: Element
    attitude_pitch: Element
    flightDirector: Element
    attitude_bank: Element
    turnRateIndicatorY: number
    turnRateIndicatorHeight: number
    turnRateIndicator: Element
    slipSkid: Element
    lowBankModeMask: Element
    lowBankMode: Element
    pitch: number
    bank: number
    slipSkidValue: number
    lowBankModeVisible: boolean
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        switch (name) {
            ...
            case 'slip_skid':
                ...
                break
            case 'background':
                if (newValue == 'false') this.backgroundVisible = false
                else this.backgroundVisible = true
                break
    ...
    parseDefinitionAttributes() {
        const isVerticalCenter = this.getAttribute('vertical-center')
        if (isVerticalCenter) {
        ...
        const turnRateIndicatorShown = this.getAttribute('show-turn-rate')
        if (turnRateIndicatorShown) {
            this.turnRateIndicatorShown = turnRateIndicatorShown == 'True'
        }
        const bottomY = this.getAttribute('bottom-y')
        if (bottomY) {
            ...
            if (angle != 0) {
                ...
                if (text) {
                    ...
                    diffAndSetAttribute(
                        leftText,
                        'y',
                        this.bankSizeRatio * angle - height / 2 + fontSize / 2 + ''
                    )
                ...
                if (angle < unusualAttitudeLowerLimit) {
                    ...
                    let path =
                        ...
                        -smallWidth / 2 +
                        ' ' +
                        (this.bankSizeRatio * nextAngle - bigHeight / 2) +
                        ' l' +
                        smallWidth +
                        '  0 '
                    path +=
                        ...
                        bigWidth / 2 +
                        ' ' +
                        (this.bankSizeRatio * angle - bigHeight / 2) +
                        ' l' +
                        -smallWidth +
                        ' 0 '
    ...
    construct() {
        ...
        }
        {
            ...
            diffAndSetAttribute(
                this.horizon,
                'viewBox',
                this.isVerticalCenter ? '-200 -150 400 300' : '-200 -200 400 300'
            )
            ...
            diffAndSetAttribute(
                this.horizonTop,
                'fill',
                this.backgroundVisible ? this.horizonTopGradient : 'transparent'
            )
            ...
            diffAndSetAttribute(
                this.horizonBottom,
                'fill',
                this.backgroundVisible ? this.horizonBottomGradient : 'transparent'
            )
        ...
        const y = this.isVerticalCenter ? -80 : -130
        ...
        const triangleOuterLeftLine = document.createElementNS(Avionics.SVG.NS, 'path')
        ...
        const triangleOuterRightLine = document.createElementNS(Avionics.SVG.NS, 'path')
        ...
        const topY = this.isVerticalCenter ? -120 : -170
        {
            ...
            const arcRadius = 126 // Doubled the radius to make the arc wider
            const startX = -106 // Adjusted to match the new width
            const startY = -radius + 60 // height
            const endX = 106 // Adjusted to match the new width
            const endY = -radius + 60 // height
            ...
            const arcD =
                ...
                ' ' +
                endY
            const arcPath = document.createElementNS(Avionics.SVG.NS, 'path')
...

        {
            if (this.turnRateIndicatorShown) {
                const turnRateIndicatorGroup = document.createElementNS(Avionics.SVG.NS, 'g')
                ...
                diffAndSetAttribute(
                    this.turnRateIndicator,
                    'height',
                    this.turnRateIndicatorHeight + ''
                )
                ...
                const leftMarker = document.createElementNS(Avionics.SVG.NS, 'rect')
                ...
                const rightMarker = document.createElementNS(Avionics.SVG.NS, 'rect')
                ...
                const centerMarker = document.createElementNS(Avionics.SVG.NS, 'rect')
        ...
        }
        {
            switch (this.slipSkidDisplayMode) {
                case SlipSkidDisplayMode.ROUND:
                    const slipSkidGroup = document.createElementNS(Avionics.SVG.NS, 'g')
                    diffAndSetAttribute(slipSkidGroup, 'id', 'slipSkid')
                    const y = this.bottomY - 30
                    ...
                    const slipSkidLeft = document.createElementNS(Avionics.SVG.NS, 'rect')
                    ...
                    const slipSkidRight = document.createElementNS(Avionics.SVG.NS, 'rect')
        ...
        {
            const radius = -topY
            const maskDef = document.createElementNS(Avionics.SVG.NS, 'defs')
            ...
            const green_arc = document.createElementNS(Avionics.SVG.NS, 'circle')
    ...
    applyAttributes() {
        ...
        if (this.slipSkid)
            diffAndSetAttribute(
                this.slipSkid,
                'transform',
                'translate(' + this.slipSkidValue * 40 + ', 0)'
            )
        if (this.horizonTop) {
        ...
        if (this.flightDirector) {
            if (this.flightDirectorActive) {
                diffAndSetAttribute(
                    ...
                    'transform',
                    'rotate(' +
                        (this.bank - this.flightDirectorBank) +
                        ') translate(0 ' +
                        (this.pitch - this.flightDirectorPitch) * this.bankSizeRatio +
                        ')'
                )
        ...
        if (this.turnRateIndicator) {
            ...
            if (turnRate < 0) {
                diffAndSetAttribute(
                    ...
                    'transform',
                    'rotate(180, 0, ' +
                        (this.turnRateIndicatorY + this.turnRateIndicatorHeight / 2) +
                        ')'
                )
            } else {
                diffAndSetAttribute(
            ...
            diffAndSetAttribute(
                this.turnRateIndicator,
                'width',
                (Math.abs(turnRate) * (this.turnRateIndicatorMarkerX / 3)).toFixed(6)
            )
        }
        if (this.lowBankModeVisible) {
            ...
            if (!isNaN(this.lowMaxBankAngle)) {
                const arcAngle =
                    Math.tan(this.lowMaxBankAngle * Avionics.Utils.DEG2RAD) * this.lowBankModeHeight
                diffAndSetAttribute(
                    this.lowBankModeMask,
                    'd',
                    'M 0 0 L ' +
                        -arcAngle +
                        ' ' +
                        -this.lowBankModeHeight +
                        ' L ' +
                        arcAngle +
                        ' ' +
                        -this.lowBankModeHeight +
