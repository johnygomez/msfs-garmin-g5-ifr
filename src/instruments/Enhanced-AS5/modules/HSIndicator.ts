...
export class HSIndicator extends HTMLElement {
    chevronBug2: any
    diamondBug2: any
    ...
    root: Element
    turnRateArc: Element
    backgroundCircle: Element
    headingBug: Element
    innerCircle: Element
    currentTrackIndicator: Element
    bearing1: Element
    bearing2: Element
    course: Element
    beginArrow: Element
    fromIndicator: Element
    CDI: Element
    endArrow: Element
    toIndicator: Element
    bearingText: Element
    ...
    navSourceBg: Element
    navSource: Element
    flightPhaseBg: Element
    flightPhase: Element
    crossTrackErrorBg: Element
    crossTrackError: Element
    dme: Element
    dmeSource: Element
    dmeIdent: Element
    dmeDistance: Element
    bearing1FixedGroup: Element
    bearing1Distance: Element
    bearing1Ident: Element
    bearing1Source: Element
    bearing2FixedGroup: Element
    bearing2Distance: Element
    bearing2Ident: Element
    bearing2Source: Element
    curPhase: number
    curDeviation: number
        ...
        switch (name) {
            ...
            case 'rotation':
                ...
                if (this.bearingText) {
                    ...
                    diffAndSetText(
                        this.bearingText,
                        '000'.slice((brg + '').length) + brg + Avionics.Utils.DEGREE_SYMBOL
                    )
                }
                break
            case 'heading_bug_rotation':
                ...
                if (this.headingText) {
                    ...
                    diffAndSetText(
                        this.headingText,
                        '000'.slice(hdg.length) + hdg + Avionics.Utils.DEGREE_SYMBOL
                    )
                }
                break
            case 'course':
                if (this.course) {
                    ...
                    if (this.courseText) {
                        ...
                        diffAndSetText(
                            this.courseText,
                            '000'.slice(crs.length) + crs + Avionics.Utils.DEGREE_SYMBOL
                        )
                    }
                }
                break
                    ...
                    if (this.turnRateArc) {
                        ...
                        if (value == 4 || value == -4) {
                            ...
                            const endPointArrowEndX =
                                50 + Math.sin(arcAngle + (value > 0 ? 0.1 : -0.1)) * arcRadius
                            const endPointArrowEndY =
                                50 - Math.cos(arcAngle + (value > 0 ? 0.1 : -0.1)) * arcRadius
                            path =
                                'M' +
                                beginPointBotX +
                                ' ' +
            ...
            case 'bearing1_source':
                if (this.bearing1Source) diffAndSetText(this.bearing1Source, newValue)
                break
            case 'bearing1_ident':
                if (this.bearing1Ident) diffAndSetText(this.bearing1Ident, newValue)
                break
            case 'bearing1_distance':
                if (this.bearing1Distance)
                    diffAndSetText(
                        this.bearing1Distance,
                        newValue == '' ? '' : fastToFixed(parseFloat(newValue), 1) + ' NM'
                    )
                break
            ...
            case 'bearing2_source':
                if (this.bearing2Source) diffAndSetText(this.bearing2Source, newValue)
                break
            case 'bearing2_ident':
                if (this.bearing2Ident) diffAndSetText(this.bearing2Ident, newValue)
                break
            case 'bearing2_distance':
                if (this.bearing2Distance)
                    diffAndSetText(
                        this.bearing2Distance,
                        newValue == '' ? '' : fastToFixed(parseFloat(newValue), 1) + ' NM'
                    )
                break
            case 'bearing2_bearing':
                ...
                break
            case 'dme_source':
                if (this.dmeSource) diffAndSetText(this.dmeSource, newValue)
                break
            case 'dme_ident':
                if (this.dmeIdent) diffAndSetText(this.dmeIdent, newValue)
                break
            case 'dme_distance':
                if (this.dmeDistance)
                    diffAndSetText(
    ...
    parseDefinitionAttributes() {
        const fmsAlias = this.getAttribute('fmsAlias')
    ...
    createSVG() {
        ...
        {
            if (!this.largeCompass) {
                const lines = [-135, -90, -45, 45, 90, 135]
        ...
        }
        {
            if (!this.noTurnRateIndicator) {
                ...
                const beginPointTopX =
                    50 - Math.sin(beginPointHalfUnitSize) * (arcRadius + arcWidth / 2)
        ...
        }
        {
            ...
            }
            {
                ...
                for (let i = 0; i < 72; i++) {
                    ...
                    diffAndSetAttribute(
                        line,
                        'transform',
                        'rotate(' + ((-angle / Math.PI) * 180 + 180) + ' 50 50)'
                    )
                    diffAndSetAttribute(line, 'fill', 'white')
                    angle += (2 * Math.PI) / 72
            ...
            if (this.displayStyle != HSIndicatorDisplayType.HUD_Simplified) {
                {
                    this.currentTrackIndicator = document.createElementNS(
                        Avionics.SVG.NS,
                        'polygon'
                    )
                    diffAndSetAttribute(
                ...
                }
                {
                    ...
                    diffAndSetAttribute(
                        arrow,
                        'd',
                        'M50 96 L50 80 M50 4 L50 20 M50 8 L57 15 M50 8 L43 15'
                    )
                ...
                }
                {
                    ...
                    diffAndSetAttribute(
                        arrow,
                        'd',
                        'M50 96 L50 92 M47 80 L47 90 Q50 96 53 90 L53 80 M50 4 L50 8 L57 15 M50 8 L43 15 M47 11 L47 20 M53 11 L53 20'
                    )
                    diffAndSetAttribute(arrow, 'stroke', 'aqua')
                    diffAndSetAttribute(arrow, 'stroke-width', '1')
                    diffAndSetAttribute(arrow, 'fill-opacity', '0')
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        diffAndSetAttribute(
                            this.endArrow,
                            'points',
                            '51,25 49,25 49,15 45,15 50,4 55,15 51,15'
                        )
                        diffAndSetAttribute(this.endArrow, 'fill', 'magenta')
        ...
        {
            const plane = document.createElementNS(Avionics.SVG.NS, 'path')
            diffAndSetAttribute(
                plane,
                'd',
                'M44 50 L49 50 L49 53 L48 54 L48 55 L52 55 L52 54 L51 53 L51 50 L56 50 L56 49 L51 48 L51 46 Q50 44 49 46 L49 48 L44 49 Z'
            )
        ...
        if (this.displayStyle != HSIndicatorDisplayType.HUD_Simplified) {
            ...
            if (!this.noCenterText) {
                ...
                const crossTrackError = document.createElementNS(Avionics.SVG.NS, 'text')
            ...
            {
                {
                    ...
                    const topLeftZone = document.createElementNS(Avionics.SVG.NS, 'path')
                ...
                }
                {
                    ...
                    const botLeftZone = document.createElementNS(Avionics.SVG.NS, 'path')
                    ...
                    {
                        const pointer1Main = document.createElementNS(Avionics.SVG.NS, 'rect')
                        ...
                        const pointer1Top = document.createElementNS(Avionics.SVG.NS, 'rect')
                        ...
                        const pointer1Bot = document.createElementNS(Avionics.SVG.NS, 'rect')
                ...
                }
                {
                    ...
                    const botRightZone = document.createElementNS(Avionics.SVG.NS, 'path')
                    botRightZone.setAttribute(
                        'd',
                        this.getExternalTextZonePath(57, Math.PI + 0.6, Math.PI + 1.1, 128, true)
                    )
                    ...
                    const pointer2 = document.createElementNS(Avionics.SVG.NS, 'path')
                    pointer2.setAttribute(
                        'd',
                        'M90 97 L92 97 M105 97 L103 97 L100 100 M103 97 L100 94 M101.5 98.5 L93 98.5 Q90 97 93 95.5 L101.5 95.5'
                    )
            ...
            case 1:
                if (Simplane.getAutopilotTacanDriven()) {
                    ...
                    diffAndSetAttribute(
                        this,
                        'course',
                        Simplane.getTacanObs(this.logic_navSelected) + ''
                    )
                    diffAndSetAttribute(
                        this,
                        'course_deviation',
                        Simplane.getTacanCdi(this.logic_navSelected) / 127 + ''
                    )
                    diffAndSetAttribute(
                        this,
                        'to_from',
                        Simplane.getTacanToFrom(this.logic_navSelected) + ''
                    )
                } else {
                    diffAndSetAttribute(
                        this,
                    ...
                    if (Simplane.getAutoPilotNavHasLoc(this.logic_navSelected)) {
                        ...
                        diffAndSetAttribute(
                            this,
                            'course',
                            Simplane.getNavLocalizer(this.logic_navSelected) + ''
                        )
                    } else {
                        ...
                        diffAndSetAttribute(
                            this,
                            'course',
                            Simplane.getNavObs(this.logic_navSelected) + ''
                        )
                    }
                    diffAndSetAttribute(
                        this,
                        'course_deviation',
                        Simplane.getNavCdi(this.logic_navSelected) / 127 + ''
                    )
                    diffAndSetAttribute(
                        this,
                        'to_from',
                        Simplane.getNavToFrom(this.logic_navSelected) + ''
                    )
                }
