export class CDI extends HTMLElement {
    ...
    parseDefinitionAttributes() {
        const noScale = this.getAttribute('no-scale')
        if (noScale) {
            this.noScale = noScale == 'True'
        }
        const indicatorShape = this.getAttribute('indicator-shape')
        if (indicatorShape) this.indicatorShape = indicatorShape
    }
    connectedCallback() {
