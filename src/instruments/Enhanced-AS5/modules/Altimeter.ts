export class Altimeter extends HTMLElement {
    ...
    currentCenterGrad: number
    minimumAltitude: number
    compactVs: boolean
    height: number
    GF_font: string
    endDigitSpace: number
    root: Element
    verticalDeviationGroup: Element
    verticalDeviationText: Element
    chevronBug: Element
    diamondBug: Element
    hollowDiamondBug: Element
    graduationGroup: Element
    graduationTexts: any[]
    ...
    digit3Top: Element
    digit3Bot: Element
    endDigitsGroup: Element
    endDigits: any[]
    bugsGroup: Element
    selectedAltitudeBug: Element
    minimumAltitudeBug: Element
    selectedAltitudeBackground: Element
    selectedAltitudeFixedBug: Element
    selectedAltText: Element
    pressureBackground: Element
    baroText: Element
    selectedVSBug: Element
    indicator: Element
    indicatorText: Element
    selectedVSBackground: Element
    selectedVSText: Element
    altitude: number
    ...
    connectedCallback() {
        ...
        diffAndSetAttribute(
            this.root,
            'viewBox',
            '-55 -100 ' + (this.compactVs ? 300 : 380) + ' ' + this.height
        )
        ...
        const centerY = this.height / 2 - 100
        {
            ...
            diffAndSetAttribute(
                this.hollowDiamondBug,
                'class',
                'vertical-deviation-hollow-diamond-bug'
            )
            diffAndSetAttribute(
                this.hollowDiamondBug,
                'points',
...

        {
            ...
            const center = (this.height - 100) / 2
            ...
            graduationSvg.appendChild(this.graduationGroup)
            {
                ...
                const n = Math.ceil((this.height - 100) / 200)
                for (let i = -n; i <= n; i++) {
                    ...
                    diffAndSetAttribute(
                        mainGrad,
                        'y',
                        fastToFixed(center - 2 + i * graduationSize, 0)
                    )
                    ...
                    diffAndSetAttribute(
                        gradText,
                        'y',
                        fastToFixed(center + 16 + i * graduationSize, 0)
                    )
            ...
            const digitGroup = document.createElementNS(Avionics.SVG.NS, 'g')
            ...
            graduationSvg.appendChild(cursorRotatingSvg)
            {
                ...
                const endDigitsScaleGroup = document.createElementNS(Avionics.SVG.NS, 'g')
...

        {
            ...
            diffAndSetAttribute(
                this.selectedAltitudeBackground,
                'class',
                'selected-altitude-background'
            )
            ...
            diffAndSetAttribute(
                this.selectedAltitudeFixedBug,
                'class',
                'selected-altitude-fixed-bug'
            )
            diffAndSetAttribute(
                this.selectedAltitudeFixedBug,
                'points',
                '10,-90 24,-90 24,-76 15,-70 24,-64 24,-50 10,-50 '
            )
                        ...
                        for (let i = 0; i < dashes.length; i++) {
                            ...
                            if (texts[i] != '') {
                                ...
                                diffAndSetAttribute(
                                    text,
                                    'y',
                                    centerY - dashes[i] - height / 2 + fontSize / 3 + ''
                                )
                ...
                default:
                    {
                        ...
                        verticalSpeedGroup.appendChild(this.selectedVSBug)
                        {
                            ...
                            verticalSpeedGroup.appendChild(this.indicator)
                            const indicatorBackground = document.createElementNS(
                                Avionics.SVG.NS,
                                'path'
                            )
                            diffAndSetAttribute(
                        ...
                        this.selectedVSBackground = document.createElementNS(
                            Avionics.SVG.NS,
                            'rect'
                        )
                        diffAndSetAttribute(
                            this.selectedVSBackground,
                            'class',
        ...
        switch (name) {
            case 'altitude':
                ...
                diffAndSetAttribute(
                    ...
                    'transform',
                    'translate(0, ' + ((value - center) * 160) / 100 + ')'
                )
                diffAndSetAttribute(
                    this.bugsGroup,
                    'transform',
                ...
                diffAndSetAttribute(
                    this.selectedAltitudeBug,
                    'transform',
                    'translate(0, ' + ((center - this.selectedAltitude) * 160) / 100 + ')'
                )
                if (!isNaN(this.minimumAltitude)) {
                    diffAndSetAttribute(
                        this.minimumAltitudeBug,
                        'transform',
                        'translate(0, ' + ((center - this.minimumAltitude) * 160) / 100 + ')'
                    )
                }
                if (this.currentCenterGrad != center) {
                    this.currentCenterGrad = center
                    const n = (this.graduationTexts.length - 1) / 2
                    for (let i = 0; i < this.graduationTexts.length; i++) {
                        diffAndSetText(
                            this.graduationTexts[i],
                            fastToFixed((n - i) * 100 + center, 0)
                        )
                    }
                }
                ...
                diffAndSetAttribute(
                    this.endDigitsGroup,
                    'transform',
                    'translate(0, ' + ((endValue - endCenter) * this.endDigitSpace) / 10 + ')'
                )
                for (let i = 0; i < this.endDigits.length; i++) {
                    const digitValue = Math.round((((2 - i) * 10 + value) % 100) / 10) * 10
                    diffAndSetText(
                        this.endDigits[i],
                        fastToFixed(Math.abs((digitValue % 100) / 10), 0) + '0'
                    )
                }
                if (Math.abs(value) >= 90) {
                    ...
                    diffAndSetText(
                        this.digit3Bot,
                        Math.abs(value) < 100 ? '' : fastToFixed(Math.floor(d3Value), 0)
                    )
                    ...
                    if (endValue > 90 || endValue < -90) {
                        if (endValue < 0) {
                            diffAndSetText(
                                this.digit3Bot,
                                fastToFixed((Math.floor(d3Value) + 1) % 10, 0)
                            )
                            diffAndSetText(
                                this.digit3Top,
                                Math.abs(value) < 100 ? '' : fastToFixed(Math.floor(d3Value), 0)
                    ...
                    if (Math.abs(value) >= 990) {
                        ...
                        diffAndSetText(
                            this.digit2Bot,
                            Math.abs(value) < 1000 ? '' : fastToFixed(Math.floor(d2Value), 0)
                        )
                        diffAndSetText(
                            this.digit2Top,
                            fastToFixed((Math.floor(d2Value) + 1) % 10, 0)
                        )
                        if ((endValue > 90 || endValue < -90) && d3Value > 9) {
                            if (endValue < 0) {
                                ...
                                diffAndSetText(
                                    ...
                                    Math.abs(value) < 1000
                                        ? ''
                                        : fastToFixed(Math.floor(d2Value), 0)
                                )
                            }
                            const translate = (endValue > 0 ? endValue - 90 : endValue + 100) * 5.7
                            diffAndSetAttribute(
                        ...
                        if (Math.abs(value) >= 9990) {
                            ...
                            diffAndSetText(
                                this.digit1Bot,
                                Math.abs(value) < 10000 ? '' : fastToFixed(Math.floor(d1Value), 0)
                            )
                            diffAndSetText(
                                this.digit1Top,
                                fastToFixed((Math.floor(d1Value) + 1) % 10, 0)
                            )
                            if ((endValue > 90 || endValue < -90) && d3Value > 9 && d2Value > 9) {
                                ...
                                const translate =
                                    (endValue > 0 ? endValue - 90 : endValue + 100) * 5.7
                                diffAndSetAttribute(
                                    this.digit1Bot,
                                    'transform',
                                    'translate(0, ' + translate + ')'
            ...
            case 'radar-altitude':
                diffAndSetAttribute(
                    ...
                    'translate(0,' +
                        Math.min(
                            (this.height - 100) / 2 + (parseFloat(newValue) * 160) / 100,
                            this.height
                        ) +
                        ')'
                )
            ...
            case 'reference-altitude':
                ...
                if (newValue != '----') {
                    ...
                    diffAndSetAttribute(
                        ...
                        'transform',
                        'translate(0, ' +
                            ((Math.round(this.altitude / 100) * 100 - this.selectedAltitude) *
                                160) /
                                100 +
                            ')'
                    )
                ...
                if (newValue != '----') {
                    ...
                    if (this.compactVs) {
                        ...
                        if (Math.abs(this.selectedVS) < 1000) {
                            ...
                            value =
                                (this.selectedVS < 0 ? -160 : 160) +
                                (this.selectedVS - (this.selectedVS < 0 ? -1000 : 1000)) / 12.5
                        }
                        ...
                        this.selectedVSBug.setAttribute(
                            'transform',
                            'translate(0, ' +
                                -Math.max(Math.min(this.selectedVS, 2250), -2250) / 10 +
                                ')'
                        )
            ...
            case 'minimum-altitude':
                ...
                if (isNaN(this.minimumAltitude)) {
                    ...
                    diffAndSetAttribute(
                        ...
                        'transform',
                        'translate(0, ' +
                            ((Math.round(this.altitude / 100) * 100 - this.minimumAltitude) * 160) /
                                100 +
                            ')'
                    )
                }
            ...
            case 'vspeed':
                ...
                if (this.compactVs) {
                    ...
                    if (Math.abs(vSpeed) < 1000) {
                        ...
                        value =
                            (vSpeed < 0 ? -160 : 160) +
                            (vSpeed - (vSpeed < 0 ? -1000 : 1000)) / 12.5
                    }
                    value = -Math.max(Math.min(value, 240), -240)
                    ...
                    this.indicator.setAttribute(
                        'transform',
                        'translate(0, ' + -Math.max(Math.min(vSpeed, 2250), -2250) / 10 + ')'
                    )
                    this.indicatorText.textContent =
                        Math.abs(vSpeed) >= 100 ? fastToFixed(Math.round(vSpeed / 50) * 50, 0) : ''
                }
                const centerY = this.height / 2 - 100
                const trendValue = Utils.Clamp(
                    centerY + (vSpeed / 10) * -1.5,
                    -50,
                    this.height - 100 - 50
                )
